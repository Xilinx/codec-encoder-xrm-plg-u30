...
bool _populate_data( const char* input, std::vector<ResourceData*> &m_resources, std::vector<ParamsData*> &m_params)
{
    ...
    try
    {
      ...
      if (pr_ptree.not_found() != pr_ptree.find("job-cnt")) 
...
int32_t xrmU30EncPlugin_calcPercent(xrmPluginFuncParam* param)
{
   ...
   {	   
      sprintf( param->output,"%d ",global_calc_enc_load);
      sprintf( temp1,"%d ",idx); 
      strcat(param->output,temp1);
   ...
   else
   {
      ...
      {	   
         sprintf( param->output,"%d ",calc_enc_aggregate);  
         sprintf( temp1,"%d ",idx); 
         strcat(param->output,temp1);	
